...
| x64/x86 linux apps | ❌ only aarch64 or arm64 apps will run depending on the cpu architecture of your phone
# Table Of Contents
...
or use this script if you have not set up pulseaudio yet [start-no-audio.sh](https://github.com/StampyMany2077/Termux-xfce4-guide/releases/download/scripts/start-no-audio.sh)   
...
to get into the ubuntu envirment you will need to do ```proot-distro login ubuntu --shared-tmp``` the ```--shared-tmp```   
is needed for the ubuntu envirment to access termux-x11 in our termux envirment.   
now install how you would *refer to [this guide](README.md#installation--run-guide) just use apt instead of pkg   
*dont you the commands to start the server you need a different one then that   
you also need to install xterm aswell to do it do ```apt install xterm```   
...
so in x11 when you run it, it tries to ping the display the display port you set just like if you were to ping a website and if you were to ping a display server that's not open yet you will see.  
